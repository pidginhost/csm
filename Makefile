...
# Deploy binary to server (local build, scp): make deploy SERVER=hostalias
deploy: build-linux
...
# Deploy + install (local build, scp): make install-remote SERVER=hostalias
...
# Upgrade existing (local build, scp): make upgrade SERVER=hostalias
upgrade: build-linux
...
# Deploy from GitLab CI artifacts: make gitlab-deploy SERVER=hostalias GITLAB_TOKEN=xxx
gitlab-deploy:
	@if [ -z "$(SERVER)" ]; then echo "Usage: make gitlab-deploy SERVER=hostalias GITLAB_TOKEN=xxx [REF=main]"; exit 1; fi
...
# Upgrade from GitLab CI artifacts: make gitlab-upgrade SERVER=hostalias
gitlab-upgrade:
	@if [ -z "$(SERVER)" ]; then echo "Usage: make gitlab-upgrade SERVER=hostalias [REF=main]"; exit 1; fi
	scp scripts/deploy.sh $(SERVER):/tmp/csm-deploy.sh
