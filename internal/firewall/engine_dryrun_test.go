...
func TestLocalAddrLookupFiltersLoopbackAndLinkLocal(t *testing.T) {
	e := &Engine{
		localAddrsLookup: func() ([]string, error) {
			return []string{"127.0.0.1", "169.254.10.20", "fe80::1", "192.0.2.40"}, nil
		},
	}
	...
	if _, ok := e.localAddrs["192.0.2.40"]; !ok {
		t.Fatal("local address cache dropped routable interface address")
	}
}
...
func TestBlockIP_VerdictAbortsOnShutdownContext(t *testing.T) {
	...
	case err := <-done:
		if err != nil {
			t.Fatalf("expected verdict ctx cancel to fail open, got %v", err)
		}
	case <-time.After(5 * time.Second):
		t.Fatal("BlockIP did not unblock after shutdown ctx cancel")
	}
}
