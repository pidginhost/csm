...
// TestRemoveTemporaryBlockDeletesOnlyAutoBlockRows pins STO-18: an operator
...
func TestRemoveTemporaryBlockDeletesOnlyAutoBlockRows(t *testing.T) {
	...
	// create this, but RemoveTemporaryBlock should still be able to clean it up.
	...
	// autoblock-sourced row either; RemoveTemporaryBlock must leave it alone.
	...
	if removed, err := db.RemoveTemporaryBlock("192.0.2.10"); err != nil || !removed {
		t.Fatalf("RemoveTemporaryBlock(192.0.2.10) = (%v, %v), want (true, nil)", removed, err)
	}
	if _, found := db.GetPermanentBlock("192.0.2.10"); found {
		t.Fatal("no-expiry auto-block row survived RemoveTemporaryBlock")
	}
	if removed, err := db.RemoveTemporaryBlock("192.0.2.11"); err != nil || !removed {
		t.Fatalf("RemoveTemporaryBlock(192.0.2.11) = (%v, %v), want (true, nil)", removed, err)
	}

	// Operator row is preserved.
	if removed, err := db.RemoveTemporaryBlock("192.0.2.12"); err != nil || removed {
		t.Fatalf("RemoveTemporaryBlock(operator) = (%v, %v), want (false, nil)", removed, err)
	}
	...
	if removed, err := db.RemoveTemporaryBlock("192.0.2.13"); err != nil || removed {
		t.Fatalf("RemoveTemporaryBlock(legacy) = (%v, %v), want (false, nil)", removed, err)
	}
	if _, found := db.GetPermanentBlock("192.0.2.13"); !found {
		t.Fatal("legacy no-source row deleted by RemoveTemporaryBlock")
	}

	// Absent IP is a no-op.
	if removed, err := db.RemoveTemporaryBlock("198.51.100.9"); err != nil || removed {
		t.Fatalf("RemoveTemporaryBlock(absent) = (%v, %v), want (false, nil)", removed, err)
	}
...
func TestRemoveTemporaryBlockLeavesOperatorUpgrade(t *testing.T) {
	db, err := Open(t.TempDir())
	if err != nil {
		t.Fatalf("Open: %v", err)
	}
	...
	removed, err := db.RemoveTemporaryBlock(ip)
	if err != nil || removed {
		t.Fatalf("RemoveTemporaryBlock(upgraded operator) = (%v, %v), want (false, nil)", removed, err)
	}
