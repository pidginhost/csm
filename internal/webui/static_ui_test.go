...
func TestEmailPageUsesPhase8Primitives(t *testing.T) {
	...
	for _, want := range []string{
		...
		`flushBackscatter`,
		`backscatter message(s) no longer queued`,
		`/api/v1/email/held`,
		`renderHeld`,
	} {
		if !strings.Contains(jsText, want) {
...
func TestSettingsPageRendersFieldsetsAndSearch(t *testing.T) {
	...
	for _, want := range []string{
		...
		"data.started_at_token || data.started_at || \"\"",
		"acceptFirstStartToken",
		"pollRestart(responseStartToken || previousStartToken, acceptFirstStartToken)",
...
// the table on a failed apply after reverting staged toggle state.
...
// unique IPs like modsec.js does, and request failures mark the affected
// chunk's cells unavailable instead of falling back to a request per cell.
