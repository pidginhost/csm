		...
		for _, failure := range []string{"backup", "copy", "chmod"} {
			t.Run(rel+"/"+failure, func(t *testing.T) {
				...
				body := strings.Join([]string{
					...
					extractShellFunction(t, script, "version_key"),
					extractShellFunction(t, script, "refuse_downgrade"),
...
func TestInstallGuardsCatchDanglingSymlinksAndFailedBootstrap(t *testing.T) {
	...
	if strings.Contains(install, "AUTH_TOKEN=$(head -c") {
		t.Error("install.sh must report the token generated by csm install, not a second unused token")
	}
	for _, want := range []string{
		...
		if !strings.Contains(install, want) {
			t.Errorf("install.sh missing required success check %q", want)
		}
	}
	patchStart := strings.Index(install, "# Patch config with detected values")
	patchEnd := strings.Index(install, "# Validate")
	if patchStart < 0 || patchEnd < patchStart || strings.Contains(install[patchStart:patchEnd], "|| true") {
		t.Error("install.sh must not suppress config patch failures")
	}
...
func TestPackagePostremovePreservesOperatorModSecurityRules(t *testing.T) {
	root := repoRootFromDaemonTest()
	body, err := os.ReadFile(filepath.Join(root, "build/packaging/scripts/postremove.sh"))
	if err != nil {
		t.Fatal(err)
	}
	script := string(body)
	start := strings.Index(script, "remove_csm_modsec_sections() {")
	if start < 0 {
		t.Fatal("postremove.sh has no marker-aware ModSecurity cleanup function")
	}
	endOffset := strings.Index(script[start:], "\n}\n")
	if endOffset < 0 {
		t.Fatal("postremove.sh ModSecurity cleanup function is malformed")
	}
	definition := script[start : start+endOffset+3]

	path := filepath.Join(t.TempDir(), "modsec2.user.conf")
	operator := "# operator before\nSecAction \"id:100001,phase:1,pass\"\n"
	managed := "# BEGIN CSM Custom ModSecurity Rules (managed by CSM - do not edit inside this block)\n" +
		"SecRule REQUEST_URI \"/xmlrpc.php\" \"id:900100,phase:1,deny\"\n" +
		"# END CSM Custom ModSecurity Rules\n" +
	...
	if writeErr := os.WriteFile(path, []byte(operator+managed+after), 0o640); writeErr != nil {
		t.Fatal(writeErr)
	}
	wrapper := filepath.Join(t.TempDir(), "cleanup.sh")
	if writeErr := os.WriteFile(wrapper, []byte("#!/bin/bash\nset -euo pipefail\n"+definition+"\nremove_csm_modsec_sections \"$1\"\n"), 0o700); writeErr != nil {
		t.Fatal(writeErr)
	}
	if output, runErr := exec.Command("/bin/bash", wrapper, path).CombinedOutput(); runErr != nil {
		t.Fatalf("postremove ModSecurity cleanup failed: %v\n%s", runErr, output)
	}
	cleaned, err := os.ReadFile(path)
	if err != nil {
		t.Fatal(err)
	}
	if string(cleaned) != operator+after {
		t.Fatalf("operator ModSecurity rules changed by postremove:\n%s", cleaned)
	}
}
