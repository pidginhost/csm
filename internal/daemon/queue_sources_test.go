...
func TestQueueSourcesDoNotHoldRegistryLockDuringSnapshot(t *testing.T) {
	...
	if got := d.QueueStatuses(); len(got) != 9 || got["late.output"].Capacity != 4 || got["first.output"].Capacity != 4 || got["actionlog.writes"].Capacity != 64 || got["phpanel.spool"].Status != "ok" || got["smtp_rdns.resolves"].Capacity != 64 || got["email_password.hashes"].Capacity != 3 || !got["email_password.waiting"].CapacityUnavailable || !got["checks.executions"].CapacityUnavailable || !got["checks.dispatch"].CapacityUnavailable {
...
func TestQueueSourcesRegistrationAndPollingAreConcurrent(t *testing.T) {
	...
	wg.Wait()
	if got := d.QueueStatuses(); len(got) != 8 || got["backend.output"].Capacity != 4 || got["actionlog.writes"].Capacity != 64 || got["phpanel.spool"].Status != "ok" || got["smtp_rdns.resolves"].Capacity != 64 || got["email_password.hashes"].Capacity != 3 || !got["email_password.waiting"].CapacityUnavailable || !got["checks.executions"].CapacityUnavailable || !got["checks.dispatch"].CapacityUnavailable {
