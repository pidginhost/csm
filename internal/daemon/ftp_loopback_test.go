...
func TestParseFTPLogLineIgnoresLoopback(t *testing.T) {
	cfg := &config.Config{}

	lines := []string{
		"pure-ftpd: (?@127.0.0.1) [INFO] user@example.com is now logged in",
		"pure-ftpd: (?@::1) [INFO] user@example.com is now logged in",
		"pure-ftpd: (?@127.0.0.1) [WARNING] Authentication failed for user [bob]",
		"pure-ftpd: (?@::1) [WARNING] Authentication failed for user [bob]",
	}
