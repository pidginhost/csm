...
// TestProbeBPFLSM_ReturnsErrorOrSucceeds is a "must not panic" smoke
// test. The probe attempts to load and attach a tiny LSM program; we
// accept either nil (kernel supports BPF LSM and the test runs as root)
// or a wrapped error (any other case). Skipped without CAP_BPF / root
// because BPF program loading is privileged.
func TestProbeBPFLSM_ReturnsErrorOrSucceeds(t *testing.T) {
	if os.Geteuid() != 0 {
		t.Skip("BPF program load requires root / CAP_BPF")
	}
	_ = probeBPFLSM() // any return is acceptable; the contract is "no panic"
}
...
// falls back to the audit listener. When the probe fails, that error
...
func TestTryStartBPFLSM_PhaseBPendingPropagates(t *testing.T) {
	...
	if os.Geteuid() == 0 {
		...
		// we are root AND the probe succeeded.
		if probeErr := probeBPFLSM(); probeErr == nil && !errors.Is(err, errBPFPhaseBPending) {
