...
func CheckSSHKeys(ctx context.Context, cfg *config.Config, store *state.Store) []alert.Finding {
	...
	for _, keyFile := range rankPathsByMtimeDesc(ctx, homes, cfg.Thresholds.AccountScanMaxFiles) {
		if ctx.Err() != nil {
			break
		}
...
func CheckAPITokens(ctx context.Context, cfg *config.Config, store *state.Store) []alert.Finding {
	...
	for _, tokenDir := range rankPathsByMtimeDesc(ctx, tokenDirs, cfg.Thresholds.AccountScanMaxFiles) {
		if ctx.Err() != nil {
			break
		}
