...
var realtimeOnlyRules = map[string]string{
	...
	"credential_mailer":               "Rename, not a gap: the same conjunction ships as credential_harvester_php, whose mail arms this rule now mirrors regex for regex. The former bare co-occurrence of mail( and the two field names measured 4 hits on the clean corpus and fired Critical on a cookie-consent plugin signing in to its vendor cloud; both engines now require a raw mail() builtin carrying the credentials",
	...
	"network_http_tunnel":             "Tier 2: realtime requires socket creation and a CONNECT request; generated wrappers and the clean corpus stay silent. No YARA counterpart; legitimate proxy clients still need evaluation before porting",
	"obfuscation_assert_string":       "Rename, not a gap: the same three assert input forms already ship as obfuscation_assert_exec",
	"obfuscation_create_function":     "Rename, not a gap: obfuscation_create_function_exec uses the same narrow direct-body signature; complex expressions and literal body analysis are deferred in signatures.md; shared positive and benign fixtures exercise both engines",
	...
	"phishing_onedrive":               "Covered and HARDENED under another name: phishing_sharepoint carries the OneDrive brand and now also reads a script submission to an off-site collector, which was the evaded arm, while this .yml form asks only for two brand words beside any typed input and already fires in realtime on live data",
	...
	"spam_hidden_div_links":           "Rename, not a gap: the anchor repetition could not cross the closing tag of a link, so the rule matched nothing. Repaired, it is the same hidden-container link farm at the same count of eight that spam_hidden_links already scans for",
	...
	"spam_pharma_generic":             "Covered under another name: spam_pharma reads the same drug list with the exact shared-anchor window; the realtime regex uses a conservative chain because RE2 cannot express that position check",
	...
	"webshell_generic_eval_request":   "Tier 2: direct request evaluation and high-confidence local flows ship as webshell_generic_passthru. The remaining arbitrary-key local arm cannot prove variable identity and joins request-selected templates to separate eval calls. Redesign before porting",
...
var renamedYARARules = map[string]string{
	...
	"obfuscation_create_function": "obfuscation_create_function_exec",
	"phishing_onedrive":           "phishing_sharepoint",
	...
	"spam_hidden_div_links":       "spam_hidden_links",
	"spam_link_injector":          "spam_wp_footer_injection",
	"spam_pharma_generic":         "spam_pharma",
