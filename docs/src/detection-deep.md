...
**It runs in a separate process, and that is not an implementation detail.** The PHP parser it depends on can be driven into an infinite loop by a crafted file -- a loop, not a crash, so it cannot be caught or timed out from inside. Analysis therefore happens in a supervised child process that is killed outright when a file exceeds its deadline; the file is reported as reduced coverage and the scan continues. If several files in a row kill their worker, the analyzer pauses briefly rather than restarting a process per file, then tries again on its own. While it is paused, and whenever a worker cannot be started, files are reported as unexamined rather than clean.
