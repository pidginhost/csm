...
| `exposed_files` | Web-downloadable sensitive files under document roots: database dumps, full-site backup archives, config/credential backups, source-code backups, and phpinfo pages. Each candidate is reported only after a headers-only reachability probe against the local vhost confirms the server serves it (HTTP 200, non-executed body) -- files the server already blocks (403) and shipped samples such as `wp-config-sample.php` are never flagged. The probe reads status and content type only, never the file body. Findings: `web_exposed_db_dump`, `web_exposed_backup_archive`, `web_exposed_config_leak`, `web_exposed_source_backup`, `web_exposed_phpinfo`. Descent depth is bounded by `thresholds.exposed_file_scan_depth` (default 2). |
