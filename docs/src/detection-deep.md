...
| `db_content` | Database injection, siteurl hijacking, rogue admins, spam. A site address is reported when its shape cannot be one -- a query string, a fragment, a script for a path, or a non-web scheme -- because WordPress builds every asset URL from it. Serving a site under a domain hosted elsewhere is ordinary and is not reported. Multisite-aware: when `wp-config.php` declares `define('MULTISITE', true)`, secondary blogs (`wp_<N>_options` / `wp_<N>_posts` for active blog IDs from `wp_blogs`) are scanned alongside the unprefixed main-site tables. |
