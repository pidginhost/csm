...
| `vulnerable_plugins` | Installed plugins whose version matches a curated known-vulnerable feed (CISA-KEV + confirmed in-the-wild CVEs). Fires only from a fresh shared inventory when a parseable version is inside the affected range; patched, stale, and unparseable versions stay silent. Matched versions are High or Critical regardless of version gap, including inactive plugins because their files remain reachable. Alert-only (never disables a plugin). Toggle `detection.vulnerable_plugin_scanning`; accept one reviewed build via `detection.vulnerable_plugin_allow` (`slug@version`, case-insensitive). Correlated with the compensating control: when the host engine is `Off` or `DetectionOnly`, or the account or vhost has ModSecurity disabled, CSM's shipped virtual patch for the CVE cannot execute, so the finding is marked unprotected in its message and raised to Critical. The disabled scope is matched against every domain of the same account served from the same document root, so a flag recorded against a vhost's servername still covers an addon domain the inventory reported under a different name. Off cPanel, where CSM deploys no virtual patches and no per-vhost ModSecurity state exists, findings are left as the detector graded them. |
