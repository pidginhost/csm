...
| `db_content` | Database injection, siteurl hijacking, rogue admins, spam, and published posts attributed to a user that does not exist (`db_phantom_post_author`, Critical above 100 posts and High below, primary blog only since the users table is network-wide in multisite). A doorway kit invents author IDs and installs a filter that removes them from dashboard queries while patching the post counts to match, so the orphaned author ID outlives every renaming of the filter. Direct WordPress installs are covered at every document root the panel serves and at every addon-domain directory in an account home, so a root the panel has stopped serving is still examined; account backup and metadata directories are not, and ordinary subdirectory installs are not discovered recursively. A site address is reported when its shape cannot be one -- a missing host, an invalid port, a backslash, a query string, a fragment, a script for a path, or a non-web scheme -- because WordPress builds every asset URL from it. Invalid-address findings are alert-only; only explicit code injection enters database auto-response. Serving a site under a domain hosted elsewhere is ordinary and is not reported. Multisite-aware: when `wp-config.php` declares `define('MULTISITE', true)`, up to 100 active secondary blogs (`wp_<N>_options` / `wp_<N>_posts` for blog IDs from `wp_blogs`) are scanned alongside the unprefixed main-site tables; larger networks emit `db_content_scan_incomplete` and retain prior findings. |
