...
| `csm doctor` | Config + daemon + watchers + store sanity check. Config checks include the firewall lockout warnings for inbound and outbound policy. On CloudLinux with PHP Shield enabled it also reports whether the Shield event mount has been applied to the cages. `csm doctor challenge` checks challenge public URL, TLS, port gate, webserver snippets, configtest, and the live `/challenge/gate` endpoint. Add `--json` for machine-readable output. |
