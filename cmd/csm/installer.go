...
thresholds:
  ...
  # Directory entries examined below one document root by the same walk.
  # A shallow tree holding very many files reaches this one first.
  php_config_walk_max_entries: 500000
...
function csm_is_command_value($value, $depth = 0) {
    if (is_array($value)) {
        foreach ($value as $item) {
            if (csm_is_command_value($item, $depth)) return true;
        }
        return false;
    }
