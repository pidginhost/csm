...
- Two critical malware rules asked only that a handful of common words appear somewhere in the same file, so a PHP archive bundling hundreds of vendor libraries matched on words drawn from unrelated files. A CGI shell rule now requires the script to actually begin as one, and the credential brute-force rule ties a password-list loop to its subsequent network call instead of accepting unrelated code paths.
