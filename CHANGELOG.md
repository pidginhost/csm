...
- The PHP taint analyzer's interprocedural pass re-checked every function and method body on every fixpoint pass, so a crafted file with a long reverse-ordered call chain took quadratic time; a single such file could burn tens of seconds per scan. It now uses a dependency worklist that only re-checks a body when a callee it actually calls just changed.
