...
- Supervisor (`internal/yaraworker.Supervisor`) that fork+execs the worker, exponential-backoff restart on crash, readiness wait via Ping, and a Scanner-shaped surface (`ScanFile`, `ScanBytes`, `Reload`, `RuleCount`) ready for the daemon to consume. Not yet wired into the daemon.
