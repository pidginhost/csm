...
- Store export is refused, before the daemon writes anything, when the destination directory or any directory above it is writable by or owned by another account, since that account could swap the archive between the digest check and the move; sticky directories such as /tmp are still accepted.
