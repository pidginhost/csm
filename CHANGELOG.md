...
- Realtime malware quarantine now honors the auto-response master switch and the quarantine opt-in, matching the scheduled quarantine path, so an operator who has not enabled file quarantine no longer has files moved on a realtime signature match.
