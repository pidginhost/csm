...
- New /api/v1/incidents/groups endpoint and a Grouped tab on the Web UI Incidents page bucket open / contained incidents by (kind, source) so a host with thousands of per-mailbox incidents browses as a handful of attacker-IP rows. Read-scope eligible; bounded scan caps at 10000 incidents per call.
