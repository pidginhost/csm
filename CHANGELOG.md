...
- A stored PHP snippet that disables caching for the request and, in the same snippet, tests the visitor against a search-engine crawler is now reported as cloaking. Neither half means anything alone, which is why both are required: caching plugins set those constants from their own files, and reading the user agent is ordinary.
