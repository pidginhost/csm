...
### Fixed
- The list of supported content management systems is now declared once and tested against the taint analyzer's path knowledge and the database scanners, so a CMS added to one without the other fails the build instead of silently reporting stock installs as compromised.
