...
- Deep scans no longer skip files that are within the size budget but too large to hand the scanner in one piece. Such files are now scanned in place instead of being reported as an unscanned gap, closing an evasion path where a payload is padded past the inline limit.
