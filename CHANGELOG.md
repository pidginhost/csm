...
- PHP content detector now treats PHP 7+ unicode-codepoint string escapes (`\u{...}`) the same way as hex escapes when scanning `call_user_func` lines for built-up function names. Attackers can no longer bypass the obfuscation check by swapping syntax.
