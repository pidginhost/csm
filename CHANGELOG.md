...
- Phishing directory heuristic no longer flags developer tutorial dumps whose HTML is a benign credential form. The fast-pass content check now requires a credential input AND at least one phishing-kit signal (external form action, self-contained inline-styled body, or brand impersonation in title or body), so trivial demo logins, contact forms, and password-reset stubs drop out without consulting any path-name allowlist.
