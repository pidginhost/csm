...
- Per-account scanners (SSH key audit, cPanel API token audit, Dovecot mailbox password audit, the Magento, Joomla, Drupal, and OpenCart database content scanners, the MySQL persistence-object scanner, and the mail forwarder / vfilter audit) now rank recently modified accounts first and stop sooner on scan cancellation, so late-alphabet accounts are no longer systematically hidden when a check timeout cuts iteration short.
