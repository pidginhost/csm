...
- Mailer signatures no longer flag stock SMTP client libraries, translation strings, JSON plugin catalogs, or a `mail()` named only in a comment; branded mailers now require a PHP open tag and a send action, and the mass-sender and relay rules require an actual per-recipient loop.
