...
- A WordPress site address rewritten to point somewhere it cannot legitimately point is now reported. WordPress builds every asset URL from that value, so an attacker who edits it makes a remote script load on every page without touching a file. The database scan previously only looked for markup or code inside the value and missed a site poisoned this way for months.
...
- An unpatched or outdated plugin no longer opens an account-compromise incident. A vulnerable version is a standing weakness, not evidence anyone got in, and reporting it as a compromise put accounts under a week-long review window on no evidence. It still alerts as a finding.
...
- The database scan only read WordPress installs directly under `public_html`, leaving sites on addon domains unscanned for injected content, poisoned addresses, and rogue administrators. On a typical shared host that was most of the sites present. They are now scanned too, so the first deep scan after upgrading may report a backlog from sites that were never covered before.
- Two critical malware rules asked only that a handful of common words appear somewhere in the same file, so a PHP archive bundling hundreds of vendor libraries matched on words drawn from unrelated files. A CGI shell rule now requires the script to actually begin as one, and the credential brute-force rule requires the password list to be iterated and used to open connections rather than merely be present.
