...
- Tightened five YAML detection rules and two YARA rules that fired on legitimate WordPress plugin code during a cPanel package restore. Each now requires its discriminator regex (or unique shell-name marker) to match instead of letting one or two generic substrings fire alone. Stops FPs on PHPMailer (`mailPassthru` + `safe_mode-aware` docblock), UpdraftPlus (`mysqldump` binary path + `wp-config.php` header reference; `popen` AJAX handlers), Elementor Pro Discord form action (`wp_remote_post` of user-configured fields), Pro Elements GitHub-updater config string, and Advanced Custom Fields REST API docblock examples. The YARA `dropper_discord_webhook` rule was identical to the YAML name but only required the webhook URL to fire, so the YAML-side tightening on its own would have left a parallel pipeline emitting Critical YARA hits on the same legitimate plugin files; it now requires the same exfiltration co-occurrence the YAML rule expects.
...
- `wpcheck` plugin checksum fetcher distinguishes a wp.org HTTP 404 (plugin is not in the repository: paid forks, custom internal plugins) from a transient error. The 404 outcome marks the slug+version not-found for 72 hours and short-circuits further retries; 5xx responses and network errors keep their normal 4-attempt backoff. Stops the periodic `plugin fetch failed for ..., retry in 1m0s: plugin zip HTTP 404` log spam without weakening checksum verification for plugins that are actually on wp.org.
- `outdated_plugins` deep finding is now aggregated per site instead of one per outdated plugin. Each site emits a single finding with severity = max of its outdated plugins, count + worst severity in the message, and the per-plugin breakdown (slug, name, installed/available versions) preserved in details. Stops a 200-account host from saturating the 500-deep alert channel buffer (and burying real signal under `alert channel full, dropping deep finding: outdated_plugins` for 17k log lines per scan cycle).
- Fanotify analyzer queue absorbs cpanel-restore-grade bursts: buffer bumped from 4000 to 16384, dirty-region tracker cap from 64 to 1024. When sustained drops still cross 500 within a minute window an out-of-cycle reconcile fires immediately instead of waiting for the minute tick, so files dropped at the start of a burst are still inside the reconcile mtime window when their parent dir gets walked.
- Three `.htaccess` detectors stop firing on legitimate plugin shapes. `htaccess_filesmatch_shield` now skips `<FilesMatch>` blocks that name specific PHP files (WebP Express `wpc.php`, named allowlists, PrestaShop `ps_facetedsearch-*.php`) and skips bare-wildcard shields when the parent directory contains multiple sibling PHP dispatchers (KCFinder). `htaccess_user_agent_cloak` skips negated UA conditions (cache-plugin social-crawler exclusion), long alternation lists (operator-installed bot blocklists), and conditions paired with a no-op or `[F]` forbid rule. `htaccess_errordocument_hijack` skips redirects to a host that shares a brand stem with the .htaccess directory path; spam-TLD targets and IP-address targets always fire.
- `new_php_in_uploads` deep-scan finding shares the cpanel-restore-staging and WP-Optimize probe recognisers used by the realtime path. The path-only gate requires a `test.php` filename under `/wp-content/uploads/wpo/` plus an installed wp-optimize plugin; an attacker dropping a webshell with a different name in the same directory still trips the suspicious-PHP-name and signature/YARA detectors that run alongside.
