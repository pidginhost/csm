...
- Link blocks a page hides from its readers are now found in the database. A container pushed off the canvas, or hidden outright, that wraps links to other domains lends the site's ranking to those domains while a visitor sees nothing; the shipped file rules for this shape never ran on injections stored in posts and options.
