...
- Restarting the daemon from the web Settings page no longer reports a spurious "Restart failed" or reloads against the still-running old daemon. The page now waits for the restarted daemon before refreshing, handles upgrades from a daemon without start markers, and reports when a restart was not observed.
