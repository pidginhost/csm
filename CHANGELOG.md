...
- The CGI-handler webshell-arming check no longer flags or auto-removes the `AddHandler cgi-script` line that Wordfence, BuddyBoss, and Magento pair with `Options -ExecCGI` to harden upload folders: that pairing disables CGI execution, so the mapping is inert. A mapping that leaves execution enabled is still flagged. This stops a false-positive loop that repeatedly edited legitimate customer `.htaccess` files.
