...
- The WP-Cron auto-fix now actually runs: when enabled it disables WP-Cron and installs a real cron during scans, instead of detecting the warning but never acting on it, so the same accounts no longer reappear in the warning list after every scan. Fixed warnings also clear from the findings list right away.
