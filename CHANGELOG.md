...
- The Exim mail-queue depth was never read on hosts where the daemon runs sandboxed: exim aborts when it cannot write its own log, and the check reported nothing on failure, so spam-outbreak detection by queue depth had been silently inactive. Queue queries now run outside the sandbox, and a queue CSM cannot read is reported instead of being treated as empty.
