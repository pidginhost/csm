...
- WordPress discovery now treats document-root resolution failures as incomplete, retains plugin inventory after partial discovery, and keeps account cleanup pointed at the primary install. A symlinked `wp-config.php` is refused rather than scanned, because wp-cli follows it as root, and incomplete database reads no longer retire earlier findings.
