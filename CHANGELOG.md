...
- Scheduled and account scans share a concurrency budget based on the machine's core count, including checks still finishing after cancellation, without reporting healthy waiting scans as stalled. The real-time scanner sizes its separate worker pool from the core count, and both packaged and installer-generated services yield CPU under contention.
