...
- Setting the connection rate limit to 0 now validates as "disabled" instead of being rejected, matching what the firewall already did with that value and what the web UI told operators it meant. Negative values are still errors, while null keeps the shipped default.
