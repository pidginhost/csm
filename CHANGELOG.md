...
- The firewall IP lookup now matches blocklist entries exactly instead of by substring, so querying one address can no longer reveal that a different, longer address is blocked. The cPanel brute-force lookup is also time-bounded, and the incident status endpoint caps its request body.
