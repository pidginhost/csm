...
- **`internal/log` package** wraps `log/slog` so operators can opt into JSON-formatted logs via `CSM_LOG_FORMAT=json` (for Loki, ELK, Datadog ingestion). `CSM_LOG_LEVEL` controls verbosity. The daemon emits the platform detection as both legacy text and structured `csmlog.Info("platform detected", "os", ..., "webserver", ...)` on startup as a reference pattern. Migration of the remaining ~190 `fmt.Fprintf(os.Stderr, ...)` call sites is incremental per subsystem.
- **Release signing infrastructure** in `.gitlab-ci.yml`. New `sign` stage produces `.sig` files for every binary, package, and asset tarball using an ed25519 key from the `CSM_SIGNING_KEY` CI variable. Install and deploy scripts now read the public key from the `EMBEDDED_SIGNING_KEY` variable (currently empty — operator provisions the key per `docs/src/release-signing.md`). Existing release behavior is preserved when no key is configured.
- **Fuzz tests for all 10 log parsers** (`parseModSecLogLine`, `parseAccessLogBruteForce`, `parseAccessLogLineEnhanced`, `parseSecureLogLine`, `parseEximLogLine`, `parseSessionLogLine`, `parseFTPLogLine`, `parseDovecotLogLine`, `parsePHPShieldLogLine`, `parseModSecLogLineDeduped`) in `internal/daemon/parsers_fuzz_test.go`. Each survives arbitrary input without panicking; ~60M total executions across all parsers found zero crashes in initial fuzzing. Run ongoing with `go test -fuzz=FuzzParseModSecLogLine -fuzztime=60s ./internal/daemon/`.
