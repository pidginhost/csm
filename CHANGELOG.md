...
- Setting the subnet-escalation threshold or the permanent-block count below two was accepted, warned about as "very low", and then silently replaced with the built-in default. Both are now rejected as configuration errors while the feature that reads them is on, the built-in defaults are applied when the keys are omitted, and the dashboard no longer offers the rejected values.
