...
- `csm doctor` now reports a binary, `csm.yaml` or conf.d hash mismatch, with the remedy, while the daemon is still running on its old hashes and before a restart refuses to start.
