...
- Phishing signatures (PayPal, Office 365, SharePoint, generic bank) now require a credential-collecting form -- a password field in a form whose action posts to a URL, path, or script -- near the brand, so plugin admin settings pages and scattered tokens in minified vendor JS no longer match.
- The "Hacked by" defacement signature now requires the phrase inside an HTML page heading, and the CoinHive miner signature requires the miner library or its instantiation rather than the bare brand, clearing false positives on security-plugin readme prose and bundled bot user-agent lists.
