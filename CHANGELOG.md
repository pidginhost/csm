...
- The PHP configuration scan stopped after a fixed number of directories below each document root and reported only that it could not finish, so on larger accounts a substantial part of the tree went unexamined every run without saying so. The limit is now configurable and much higher by default, and the report names the limit, how far the scan got, and the setting that raises it.
