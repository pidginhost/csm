...
- Every web UI page now routes its API calls through the shared request helper, so a hung backend can no longer freeze a tab indefinitely. All GETs and the live-data poller share the same 30-second timeout.
