...
- SIGHUP config hot-reload (ROADMAP item 7, initial). `internal/config` gains `Active()` / `SetActive()` around an `atomic.Pointer[Config]`, plus a `Diff()` that classifies top-level Config fields via a new `hotreload` struct tag. Fields tagged `"safe"` swap in place on SIGHUP; fields tagged `"restart"` (and untagged fields, which default to restart) cause the reload to abort with a Warning `config_reload_restart_required` finding listing the offending field names, leaving the live config untouched. `Thresholds` is the first field tagged safe: editing a threshold and sending `kill -HUP $(pidof csm)` now takes effect on the next tick without dropping fanotify marks. Parse/validation failures emit a Critical `config_reload_error` and keep the old config live. The reload re-signs `integrity.config_hash` in place (mirroring `csm rehash`), so a scripted edit + SIGHUP no longer requires the manual rehash step. Further fields (alerts, suppressions, reputation, metrics_token, signature rule paths) stay on restart in this release; each will be flipped to safe in follow-ups as the readers migrate to `config.Active()`.
