...
- `/metrics` endpoint on the web UI server, ROADMAP item 4 first iteration. Auth accepts `cfg.WebUI.MetricsToken` as a dedicated Bearer (so Prometheus scrapers get a credential that does not also unlock the UI) and falls back to the existing UI AuthToken / session cookie so the dashboard can self-scrape. Read-only, no CSRF, `text/plain; version=0.0.4`. Shipped metrics: `csm_build_info{version}`, `csm_yara_worker_restarts_total` (when worker mode is on), `csm_findings_total{severity}` (incremented from `state.AppendHistory`), `csm_store_size_bytes` (stats the bbolt file at scrape time). `docs/src/metrics.md` documents the scrape config (passes `promtool check config`). Further instrumentation (fanotify queue, check durations, firewall/blocked-IP counts, auto-response actions) lands in follow-ups.
