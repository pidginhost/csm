...
- A scan that could not read a few specific files now still retires findings for every file it did read. Coverage was tracked per scan, so one permanently unreadable file -- an error log past the scan limit, which will never fit -- marked the whole scanner partial on every cycle and froze every finding it had ever raised. A gap that names a file protects only that file; a gap with no path, such as a directory the walk could not enter, still holds the whole scanner's findings, because the unscanned range is genuinely unknown.
