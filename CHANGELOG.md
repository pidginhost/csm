...
- Database scanning now covers WordPress sites on addon domains, so the first deep scan may report findings from sites that were not previously checked. It uses cPanel's served-site map, fewer queries, and bounded cancellable work to avoid scanning account backups or overrunning the deep-scan cycle.
