...
- Saving the scheduled-scan disabled-checks list now rejects unknown check names instead of accepting them silently, so a typo no longer leaves a check running while it appears disabled. Existing entries that use a runner ID still validate.
