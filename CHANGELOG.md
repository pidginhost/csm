...
- A batch of malware signatures now require executable context: mu-plugin, symlink, goto-obfuscation, reverse-shell, cloaking, and core-modification rules key on a PHP (or Perl) open tag and word-boundaried sinks rather than substrings, so `doubleval()`, TypeScript and C++ sources, PHP error logs, HTML help pages, and stock HTTP clients no longer match. Pharma-spam and footer-injection rules now require the drug term, call to action, and hide-it style to sit together, and the fake-ionCube rule requires its decode call next to the marker.
