...
- Quarantine restore now sets the requested mode before changing ownership, so the new owner never observes a wider-than-intended mode during the brief window between the two syscalls. Mode and ownership errors are now logged instead of silently swallowed.
