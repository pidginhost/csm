...
- Web UI CORS check now derives the allowed origin from configuration instead of the request's Host header. A proxy attacker can no longer pair a forged Host with a matching Origin to bypass the cross-origin check.
