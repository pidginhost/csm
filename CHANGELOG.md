...
- Auto-block tracker, perm-block tracker, and firewall state file now write through a shared atomic helper that fsyncs the temp file before rename. A daemon crash or host power-loss mid-write can no longer leave a torn JSON file that the next start would fail to parse.
