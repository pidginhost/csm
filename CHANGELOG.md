...
- A script writing PHP into the plugins directory is now reported only when the file is PHP source. Scans flagged PHP error logs, which quote the same failed write on a single line.
