...
- Finding history and attack event logs use less database space for mostly chronological arrivals and avoid excessive page splitting when delayed records dominate a write. Space savings depend on record sizes and arrival order; existing files can reclaim unused space at compaction.
