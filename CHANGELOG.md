...
- User-Agent spoof detector no longer fires on IPs that have no reverse DNS at all; only PTR records that resolve outside the claimed bot's published domain still count as spoof. Amazonbot is now recognised on Amazon's `.amazon` top-level domain, so legitimate `*.crawl.amazonbot.amazon.` crawlers stop paging operators as critical.
