...
- Optional Sentry integration for crash reporting and selective error capture. New `internal/obs` package wraps `sentry-go` v0.45.1 and a `sentry:` config block (`enabled`, `dsn`, `environment`, `sample_rate`, `debug`) turns it on; disabled and empty-DSN are both no-ops. Long-lived daemon goroutines (scanners, dispatchers, watchers, fanotify workers, PAM/control listeners, webui, yara-worker supervisor, firewall refresh) run via `obs.Go` / `obs.SafeGo` so a panic lands in Sentry with `component`, `os`, `panel`, and `webserver` tags before the process crashes. DSN is redacted from `csm config show`.
...
- Emailav YARA-X adapter now works under worker mode, closing ROADMAP item 2's last open sub-item. `yaraipc.Match` and `yara.Match` gained `Meta map[string]string` (string-valued rule metadata extracted from `rule.Metadata()` inside the worker), and `emailav.YaraXScanner` was rewritten to take a `yara.Backend` and read severity from `Meta["severity"]` instead of reaching for the compiled `*yara_x.Rules` across a process boundary. Non-string metadata is dropped at the worker — deliberate policy, not a fidelity claim. Daemon and email-API UI also switched from `yara.Global()` to `yara.Active()` for forge-update and rule-count reporting so every YARA-X consumer behaves the same under in-process and worker backends.
...
- SIGHUP config hot-reload (ROADMAP item 7, initial). `internal/config` gains `Active()` / `SetActive()` around an `atomic.Pointer[Config]`, plus a `Diff()` that classifies top-level Config fields via a new `hotreload` struct tag. Fields tagged `"safe"` swap in place on SIGHUP; fields tagged `"restart"` (and untagged fields, which default to restart) cause the reload to abort with a Warning `config_reload_restart_required` finding listing the offending field names, leaving the live config untouched. `Thresholds` is the first field tagged safe: editing a threshold and sending `kill -HUP $(pidof csm)` (or `systemctl reload csm`) now takes effect on the next tick without dropping fanotify marks. Parse/validation failures emit a Critical `config_reload_error` and keep the old config live. The reload re-signs `integrity.config_hash` in place (mirroring `csm rehash`), so a scripted edit + SIGHUP no longer requires the manual rehash step. `ExecReload=/bin/kill -HUP $MAINPID` wired into the systemd unit (`cmd/csm/installer.go` and `build/packaging/systemd/csm.service`). `docs/src/configuration.md` "Editing csm.yaml by hand" rewritten with the fast-path (SIGHUP reload) and restart-path (rehash + restart) flows plus Ansible/Puppet/Chef guidance. Known caveat documented: four threshold values (`deep_scan_interval_min`, `wp_core_check_interval_min`, `webshell_scan_interval_min`, `filesystem_scan_interval_min`) drive long-lived tickers set at daemon startup; they hot-reload cleanly in terms of stored value and diff, but the ticker keeps firing at the old interval until restart. Every other threshold (rates, counts, windows, severity cutoffs) is read per-tick inside checks and takes effect on the next tick. Further fields (`alerts`, `suppressions`, `reputation`, `webui.metrics_token`, `signatures.rules_dir`) stay on restart in this release; each will be flipped to safe in follow-ups as the readers migrate to `config.Active()` -- tracked on ROADMAP item 7.

### Fixed

- False-positive quarantine storm on WP auto-updates and WPML: deep-scan path for `/wp-content/languages/` and `/wp-content/upgrade/` now runs content analysis first (mirroring the realtime path fixed earlier), WPML translation queue (`/wp-content/languages/wpml/queue/`) is recognised as known-safe, and the high-confidence auto-quarantine entropy floor is raised from 4.8 to 5.5 (with WPML and Breakdance added to the library-path allowlist). An incident on 2026-04-17 quarantined 109 benign files; the same content no longer trips any gate.
- Realtime `php_in_sensitive_dir_realtime` Warning no longer fires on WPML translation-queue regenerations. The previous WPML allowlist was wired into the polled deep-scan only; the fanotify path now shares the same `checks.IsSafePHPInWPDir` helper, so a single language-pack refresh no longer produces 30-40 spurious Warnings per second.
- Realtime fanotify tests in the `daemon` package no longer feed `os.File.Fd()` into `analyzeFile`/`checkCrontab`/`checkPHPContent`/`checkHtaccess`/`checkUserINI`/`checkHTMLPhishing`/`checkCGIBackdoor`; that pattern adds the netpoller and a GC finalizer to fds the test then hands off to syscalling code, and produced an intermittent EBADF in `t.TempDir` cleanup on `TestCheckCrontab_Base64WrappedFires` under `-race` + coverage (CI job 93822). The full sweep — 11 test files via a shared `openRawFd` helper — uses raw `unix.Open`/`unix.Close` fds, matching the production fanotify ownership model.
...
- SIGHUP reload no longer misclassifies every reload as `restart_required` because of a false-positive firewall diff. `startFirewall` merged `cfg.InfraIPs` into `d.cfg.Firewall.InfraIPs` at daemon startup, mutating the running config; a later reload loaded a fresh config whose `firewall.infra_ips` had NOT been merged, and `reflect.DeepEqual` then flagged the firewall subtree as changed even on an unedited file. The merge now builds a shallow-copied `FirewallConfig` for `firewall.NewEngine` and leaves `d.cfg.Firewall` untouched. Caught on the cluster6 live smoke (2026-04-19). Regression guarded by `TestDiffLoadLoadIsEmpty` which asserts two consecutive `config.Load()` calls on the same file produce zero diff.
- A SIGHUP reload that is rejected as `restart_required` no longer leaves the on-disk file with a stale `integrity.config_hash`. Previously the edited file stayed on disk with the pre-edit stored hash, so any systemd/manual/crash-recovery restart would fail `integrity.Verify` on startup and crash-loop. The reload handler now re-signs the on-disk file via `integrity.SignAndSaveAtomic` on the restart_required branch and updates the live `config.Active()` ConfigHash in lock-step so periodic `integrity.Verify(currentCfg)` does not observe a disk/memory divergence and fire spurious tamper alerts between the edit and the eventual restart. Also caught on the cluster6 live smoke. Regression guarded by `TestReloadConfigRestartRequiredKeepsIntegrityConsistent`.
- `firewall: warning loading state: netlink receive: recvmsg: no buffer space available` on startup and every SIGHUP. `Engine.loadState` issued one `SetAddElements` call per persisted entry, so hosts with a few hundred persisted blocks overflowed the netlink socket's `SO_RCVBUF` when `Flush` drained the ack stream. Entries are now collected into one slice per target set (six slices total) and each slice is shipped via a new `addElementsChunked` helper that caps each netlink message at 1000 elements (~28 KB worst-case, safely below the default 208 KB rmem). The 1000-element chunk is even so interval sets (blocked_net) never split a `{start, IntervalEnd}` pair across chunks. Scales to arbitrarily large blocklists without reintroducing ENOBUFS.
- Firewall subnet-routing helper `resolveSubnetSet` and `loadState`'s blocked_net loop now skip cleanly when `lastIPInRange` returns nil (malformed `net.IPNet` whose IP is neither 4 nor 16 bytes). Prior code fell through to `nextIP(nil)` and would feed an empty Key to the kernel. The three production callers (`reapExpired`, `AddBlockedSubnet`, `RemoveBlockedSubnet`) already gate on `set != nil`, so propagating the nil through the helper short-circuits them without new checks. Regression guarded by `TestResolveSubnetSetMalformedIPReturnsNil`.
- `plugincheck` no longer double-logs one hung wp-cli call as both `Command timed out: su [...]` and `plugincheck: JSON parse failed: unexpected end of JSON input`, and stderr chatter from wp-cli (PHP warnings, MySQL deprecation notices, broken third-party plugin backtraces) can no longer poison the JSON we parse (the `invalid character 'W'/'P'/'N' looking for beginning of value` class). wp-cli now runs through a new stdout-only `CmdRunner.RunContextStdout` helper that surfaces `context.DeadlineExceeded` to callers, with `--skip-plugins --skip-themes` and `WP_CLI_PHP_ARGS='-d display_errors=0 -d error_reporting=0'` so one broken plugin on one site cannot tip the whole enumeration into exit 255; per-site failure logs roll up into one `plugincheck: refresh ... (timeout=N exec_fail=N json_fail=N)` summary. Regression guarded by `TestRefreshPluginCacheTimeoutDoesNotDoubleLog`, `TestRefreshPluginCacheDropsStderrFromStdout`, and `TestRunCmdStdoutContextDropsStderr` / `TestRunCmdStdoutContextTimeoutReturnsDeadlineExceeded`.

### Changed

- SIGHUP hot-reload widens the safe-reload set from one field to six. `alerts`, `suppressions`, `auto_response`, `reputation`, and `email_protection` are now tagged `hotreload:"safe"` alongside `thresholds`. Daemon auto-response paths (`AutoBlockIPs`, `AutoFixPermissions`, `AutoKillProcesses`, `AutoQuarantineFiles`, `AutoRespondDBMalware`, `ChallengeRouteIPs`), alert dispatch (`alert.Dispatch`), and the heartbeat (`alert.SendHeartbeat`) now read `d.currentCfg()` per call so a SIGHUP-driven edit to any of those sections takes effect on the next tick. Batch handlers (`dispatchBatch`, initial-scan dispatch) snapshot `d.currentCfg()` once at the top of the batch so a reload landing mid-batch never splits a single finding set between old and new policy. Regression guard `TestDiffAllSafeFieldsClassifiedSafe` fails immediately if a future refactor drops any of these tags.
