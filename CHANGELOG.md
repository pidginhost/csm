...
- Added real-time PHP content-scan truncation telemetry so operators can see when large files exceed the scan window before tuning limits. The same counter now also covers the per-user crontab, per-vhost .htaccess, and per-vhost .user.ini real-time checks.
