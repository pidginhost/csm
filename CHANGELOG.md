...
- Cleanup-history backend for the new MySQL persistence drops. Two new endpoints: `GET /api/v1/db-object-backups` lists every record in the `db_object_backups` bbolt bucket (newest first, with the bbolt key as the opaque round-trip handle); `POST /api/v1/db-object-backup-restore` re-executes the captured CREATE SQL to restore a previously-dropped trigger / event / procedure / function. CSRF-protected. The browser-side dashboard tab is deferred to a session where I can drive a browser; the API is fully functional via curl today.
- Hardened `.htaccess` pre_clean backups now appear in the existing `/api/v1/quarantine` listing because the cleaner writes its `.meta` sidecar in the same JSON shape `QuarantineMeta` uses across the rest of the codebase. Operators can roll back htaccess cleans through the existing `/api/v1/quarantine-restore` handler with no new code path. The previous key=value sidecar format was readable by no consumer in the pipeline.
- OpenCart database content scanning. New `db_content_opencart` deep-tier check discovers OpenCart installs by globbing `config.php` and confirming both it and `admin/config.php` contain `define('DB_DRIVER'` (the admin-side pair distinguishes OpenCart from arbitrary PHP sites with a root-level `config.php`). Credentials parsed from the canonical `DB_HOSTNAME` / `DB_USERNAME` / `DB_PASSWORD` / `DB_DATABASE` / `DB_PREFIX` defines via the existing `extractDefine` helper. Scans `<prefix>setting` (with `config_url` / `config_ssl` as canonical hijack targets), `<prefix>product_description`, `<prefix>information_description`, and `<prefix>user` (admin/staff accounts; customers live in `oc_customer` and are out of scope). Three new finding categories: `opencart_settings_injection`, `opencart_content_injection`, `opencart_admin_injection`.
- Magento 1.x and 2.x database content scanning. New `db_content_magento` deep-tier check probes `app/etc/env.php` (M2, preferred) and `app/etc/local.xml` (M1) for credentials. M1 XML is parsed via `encoding/xml` (CDATA and bare-text fields both handled); M2 PHP is parsed via field-level regex (single or double-quoted keys). Tables identical between versions: `core_config_data` (settings — `web/unsecure/base_url` is the canonical hijack target), `catalog_product_entity_text`, `cms_block`, `cms_page` (content), and `admin_user` (admin accounts). Three new finding categories: `magento_settings_injection`, `magento_content_injection`, `magento_admin_injection`. Hosts carrying both M1 and M2 files (mid-migration) only scan once via the M2 path.
- Drupal 8+ database content scanning. New `db_content_drupal` deep-tier check discovers Drupal sites by globbing `sites/default/settings.php` and confirming `core/lib/Drupal.php` exists in the site root. Credentials parsed from the `$databases` array via regex (both `array(...)` and short `[...]` syntax). Scans `config` (settings), `node_revision__body` (article content), and `users_field_data` joined with `user__roles` (administrator role detection). Three new finding categories: `drupal_settings_injection`, `drupal_content_injection`, `drupal_admin_injection`. Drupal 7 (EOL January 2025) is out of scope; lands as a follow-up if any operator reports D7 sites in production.
- Joomla database content scanning. New `db_content_joomla` deep-tier check discovers Joomla installs by reading `/home/*/public_html/configuration.php` for the `class JConfig` marker, parses credentials from `public $foo = '...';` assignments via line-anchored regex (no PHP eval), and scans the three canonical attacker-touched tables: `<prefix>extensions` (params blob), `<prefix>content` (article body), and `<prefix>users` joined with `<prefix>user_usergroup_map` for Super User accounts (group_id=8). Three new finding categories: `joomla_extensions_injection`, `joomla_content_injection`, `joomla_admin_injection`. Drupal, Magento, and OpenCart adapters land in follow-up commits.
- WordPress multisite database scanning. The `db_content` deep-tier check now detects `define('MULTISITE', true)` in `wp-config.php`, queries `wp_blogs` for active secondary blog IDs (excluding archived, deleted, and spam blogs), and scans each one's `wp_<N>_options` and `wp_<N>_posts` tables alongside the main-site (unprefixed) tables. `wp_users` / `wp_usermeta` stay scanned once because WordPress shares them across the network by default.
- Signature-update-driven retroactive deep rescan. A new daemon goroutine polls `signatures.rules_dir` every 60s and arms a one-shot rescan flag when any `*.yaml` / `*.yml` / `*.yar` / `*.yara` file's mtime advances. The next deep-tier tick reads the flag and sweeps the full account tree against the new ruleset, so an `update-rules` install catches existing on-disk files, not just files that change after the update. mtimes are persisted in bbolt so a daemon restart does not trigger a phantom rescan. New `csm_signature_rescans_total` metric and a `signature_update_rescan_queued` finding per changed file. Toggle: `detection.rescan_on_signature_update` (default on).
