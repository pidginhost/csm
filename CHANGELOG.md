...
- The stale-WAF-rules warning measured the oldest ruleset on disk, so a host that had switched vendors was told its rules were a month old while the vendor actually in use was updating daily. Retired vendor rule trees stay on disk forever and no longer decide this; the warning now reflects when any installed ruleset was last refreshed.
