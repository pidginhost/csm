...
- A critical finding no longer carries the warnings batched with it past the hourly alert limit, and critical alerts no longer use up that limit.
- Failed routine alert deliveries no longer consume the hourly limit when only an urgent email succeeds, leaving the budget available for a later delivery.
