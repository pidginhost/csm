...
- WordPress discovery now treats document-root resolution failures as incomplete, retains plugin inventory after partial discovery, and keeps account cleanup pointed at the primary install. Symlinked configuration files remain visible to core and plugin checks, and incomplete database reads no longer retire earlier findings.
