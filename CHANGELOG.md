...
- `/metrics` endpoint on the web UI server, ROADMAP item 4 first iteration. Auth accepts `cfg.WebUI.MetricsToken` as a dedicated Bearer (so Prometheus scrapers get a credential that does not also unlock the UI) and falls back to the existing UI AuthToken / session cookie so the dashboard can self-scrape. Read-only, no CSRF, `text/plain; version=0.0.4`. Ships with two registered metrics out of the box: `csm_build_info{version}` (value always 1, version in the label) and `csm_yara_worker_restarts_total` (counter hook onto `yaraworker.Supervisor.RestartCount()` when worker mode is enabled). Further instrumentation (findings, fanotify queue, check durations, store size, firewall/blocked-IP counts, auto-response actions) lands in a follow-up.
