...
- Store export is refused when the destination directory is writable by, or owned by, another account, since that account could swap the archive between the digest check and the move; a sticky directory such as /tmp is still accepted.
