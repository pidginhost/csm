...
- WordPress translation and core updates no longer raise critical self-deleting file alerts when the updater copies a staged file into place and removes the original. A file removed from the same staging paths without an identical installed copy is still reported.
