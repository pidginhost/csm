...
- PHP scanning no longer skips files by name or location in WordPress language, mu-plugins, and bundled-dependency directories, so a backdoor hidden under a translation-style filename or inside a vendor folder is now content-analysed like any other file. A file that cannot be read fails closed instead of being treated as clean.
