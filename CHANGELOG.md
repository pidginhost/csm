...
- Auto-block now treats every mailbox-takeover-class realtime check as blockable: `email_auth_failure_realtime`, `email_credential_leak`, `email_rate_critical`, `email_spam_outbreak`, `email_suspicious_geo`, `mail_account_spray`, `smtp_account_spray`, and the `credential_spray` super-incident. Previously a sustained mailbox-spray flood from one IP could rack up hundreds of incidents without the firewall ever seeing the attacker.
...
- Phishing directory heuristic now walks parent directories before flagging. A developer's tutorial dump like `/home/u/.../temp/JavaScript Login/vers-1` was hitting `phishing_directory` because the immediate dir name slipped past the business-name check; the parent `temp` and grandparent `JavaScript Login` are now inspected and reject scratch / tech-prefix ancestors.
