...
- Restarting the daemon from the web Settings page no longer reports a spurious "Restart failed": the restart is acknowledged before it runs, so the browser sees success and reconnects once the daemon is back, instead of seeing the server terminate the request mid-flight.
