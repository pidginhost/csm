...
- Malware content scans no longer flag signature tokens inside compressed archive files (plugin backup `.zip`, `.gz`, `.bz2`, `.xz`, `.7z`, `.rar`) under any scan backend, including the daemon's out-of-process worker. Both signature engines skip only recognized archive headers; tar and executable PHAR content stay scannable, existing identical findings are re-checked after upgrade, and payloads are scanned when extracted to disk.
