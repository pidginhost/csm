...
- Malware detection now catches two more webshell forms from the same cross-account compromise: an eval() loader that breaks out of PHP with a "?>" prefix before decoding its payload (which slipped past the existing decoder-chain rule), and a dropped command-console tool that gates its own shell behind a login so the request-to-sink rules never fired.
