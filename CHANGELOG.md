...
- Realtime fanotify tests in the `daemon` package no longer feed `os.File.Fd()` into `analyzeFile`/`checkCrontab`/`checkPHPContent`/`checkHtaccess`/`checkUserINI`/`checkHTMLPhishing`/`checkCGIBackdoor`; the runtime poller's documented "may close the fd at unspecified times" behaviour collided with `t.TempDir` cleanup under `-race` + coverage and produced an intermittent EBADF on `TestCheckCrontab_Base64WrappedFires`. The full sweep — 11 test files via a shared `openRawFd` helper — uses raw `unix.Open`/`unix.Close` fds, matching the production fanotify ownership model.
